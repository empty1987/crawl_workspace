...
//		}
	...
	public static String pageMultTag(String action, Page page, Map params){
		...
		int pageSize = page.getPageSize();
		StringBuffer sb = new StringBuffer();
		sb.append("<div class='pages-nav' align='right' ><span class='text1'>");
		sb.append("共");
		sb.append(totalCount);
		sb.append("条记录 ");
		...
		sb.append("<input type='text' id='gotopage' size='3' onkeypress='respKeyEvent(event);'/></span><a href='#nogo' class='text1' onclick='jumpPage();'>GO</a>");
	...
	public static String pageMultTag1(String action, Page page, Map params){
		long pageCount = page.getTotalPages();
		long totalCount = page.getTotalCount();
		long pageNo = page.getPageNo();
		int pageSize = page.getPageSize();
		int resultSize = page.getResult().size();
		StringBuffer sb = new StringBuffer();
		sb.append("<div class='pages-nav'><span class='text'>");
		sb.append("第"+pageNo+" 页</span>");
		long prePage = pageNo - 1;
		if(prePage < 1)
			prePage = 1;
		if (pageNo>1) {
