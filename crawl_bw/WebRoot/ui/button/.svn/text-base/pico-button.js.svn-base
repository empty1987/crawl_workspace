...
(function($){
			...
			var bntStr=[
				'<table id="',id,'" class="z-btn" cellSpacing=0 cellPadding=0 border=0><tbody><tr>',
					...
					'<TD class=z-btn-center><EM unselectable="on">',
						'<INPUT type="button" class="z-button z-btn-text"  value="',title,'"></INPUT>',
					'</EM></TD>',
	...
	$(function(){
		$("input[z-type='button']").each(function(){
			$(this).btn().init();
		
		});
		$("input[z-type='reset']").each(function(){
		...
		$("input[z-type='submit']").each(function(){
			$(this).btn().init().click(function(){
				var form = $(this).parents("form")[0];
				if(form)
					form.submit();
